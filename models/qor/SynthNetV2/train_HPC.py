...
datasetDict =  {
    ...
    'bad_mixmatch' : ["train_data__bad_mixmatch.csv","test_data__bad_mixmatch.csv"],
    'bad_set2' : ["train_data__bad_set2.csv","test_data__bad_set2.csv"]
}
...
DUMP_DIR = osp.join('/scratch/abc586/OpenABC-dataset/SynthV16_AND',RUN_DIR)
...
SYNTH_MODEL = SynthNet(node_encoder=node_encoder,synth_encoder=synthesis_encoder,n_classes=num_classes,synth_input_dim=synthFlowEncodingDim,node_input_dim=nodeEmbeddingDim)
optimizer = torch.optim.Adam(SYNTH_MODEL.parameters(),lr=learning_rate)
scheduler = ReduceLROnPlateau(optimizer, 'min',verbose=True)
device = getDevice()
SYNTH_MODEL = SYNTH_MODEL.to(device)
...
full_train_dl = DataLoader(trainDS,shuffle=True,batch_size=batchSize,pin_memory=True,num_workers=4)
...
for ep in range(1, num_epochs + 1):
    ...
    print("\nTraining..")
    trainLoss = train(SYNTH_MODEL, device, train_dl, optimizer)

    print("\nEvaluation..")
    validLoss = evaluate(SYNTH_MODEL, device, valid_dl)

    print({'Train loss': trainLoss,'Validation loss': validLoss})
    valid_curve.append(validLoss)
    train_loss.append(trainLoss)
    torch.save(SYNTH_MODEL.state_dict(), osp.join(DUMP_DIR, 'gcn-epoch-{}-val_loss-{:.3f}.pt'.format(ep, validLoss)))
    scheduler.step(validLoss)

best_val_epoch = np.argmax(np.array(valid_curve))
testAcc = evaluate(SYNTH_MODEL, device, test_dl)
